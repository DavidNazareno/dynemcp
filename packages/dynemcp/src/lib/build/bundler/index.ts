...
export async function bundle(options: BundleOptions): Promise<BundleResult> {
  ...
  try {
    ...
    if (buildResult.warnings.length > 0) {
      ...
      buildResult.warnings.forEach((warning) => console.warn(`  ${warning.text}`));
    }
 ...
 */
export async function bundleWatch(options: BundleOptions): Promise<BuildContext> {
...
export async function cleanBuildDir(outDir: string): Promise<void> {
  ...
  const absoluteOutDir = path.isAbsolute(outDir) ? outDir : path.join(process.cwd(), outDir);
