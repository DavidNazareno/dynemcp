...
export class SSETransport implements Transport {
  private config: NonNullable<TransportConfig & { type: 'sse' }>['options'];
  private server?: any;

  constructor(config: NonNullable<TransportConfig & { type: 'sse' }>['options'] = {}) {
    this.config = {
      port: 8080,
      endpoint: '/sse',
...
export class HTTPStreamTransport implements Transport {
  private config: NonNullable<TransportConfig & { type: 'http-stream' }>['options'];
  private server?: any;

  constructor(config: NonNullable<TransportConfig & { type: 'http-stream' }>['options'] = {}) {
    this.config = {
      port: 8080,
      endpoint: '/mcp',
      responseMode: 'batch',
