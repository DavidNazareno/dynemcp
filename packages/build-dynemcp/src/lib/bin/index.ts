...
async function run(): Promise<void> {
  try {
    ...
    if (!fs.existsSync(absoluteConfigPath)) {
      ...
      console.log(chalk.yellow('💡 Make sure you are in a DyneMCP project directory or specify a config file with --config'));
      process.exit(1);
    }
...
// Execute the CLI
run().catch((error) => {
  console.error(chalk.red(`❌ CLI failed: ${error instanceof Error ? error.message : String(error)}`));
  process.exit(1);
});
