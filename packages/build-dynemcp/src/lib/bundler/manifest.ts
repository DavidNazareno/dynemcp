...
export async function generateManifest(metafile: Metafile, outDir: string): Promise<void> {
  try {
    ...
    manifest.entryPoints = Object.keys(metafile.inputs).filter(path => 
      !path.includes('node_modules/') && path.endsWith('.ts')
    );
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                <h3>📁 Output Files</h3>
                <div class="file-list">
                    ${Object.entries(manifest.outputs).map(([file, info]) => `
                        <div class="file-item">
                            <span>${file}</span>
            ...
            <div class="section">
                ...
                <div class="dependency-list">
                    ${Object.entries(manifest.dependencies).map(([name, info]) => `
