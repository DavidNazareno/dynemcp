...
export class AdvancedCalculatorTool extends DyneMCPTool<
  typeof AdvancedCalculatorSchema
> {
  name = 'advanced-calculator'
  description =
    'Perform advanced mathematical operations (power, sqrt, log, sin, cos, tan)'
  ...
  async execute({
    ...
    base = 10,
    exponent,
  }: z.infer<typeof AdvancedCalculatorSchema>) {
    let result: number
  ...
  private formatOperation(
    ...
    switch (operation) {
      case 'power':
        return `${value}^${exponent} = ${Math.pow(
          value,
          exponent !== undefined ? exponent : 1
        )}`
      case 'sqrt':
        return `√${value} = ${Math.sqrt(value)}`
      case 'log':
        return `log_${base}(${value}) = ${
          Math.log(value) / Math.log(base !== undefined ? base : 10)
        }`
      case 'sin':
